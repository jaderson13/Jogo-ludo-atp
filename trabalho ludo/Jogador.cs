...
namespace trabalho_ludo
{
    internal class Jogador
    {
        ...
        public bool Validar_jogada(int jogada_escolhida, int peao_escolhido_coluna, out bool sair_da_casa,out bool peao_vitoria)
        {
            sair_da_casa = false;
            ...
            for(int i = 0; i < this.vet_peoes.Length; i++)
            {
                for(int j = 0; j < this.jogadas.Length; j++)
                {
                    if (this.jogadas[j] != 0)
                    {
                        ...
                        {
                            if (this.jogadas[j] == 6) { return false; }
                        }

                        else if (this.vet_peoes[i].coluna + this.jogadas[j] <= 56) { return false; }
                    }
                }
        ...
        public bool Jogar_dado(ref bool jogada_extra,Jogo ludo)
        {
            ...
            Console.WriteLine("\nJogar dado!");
            ludo.log_partida.WriteLine($"Jogar dado!");
        ...
        public int[] Exibir_posicoes()
        {
            int[] posicoes = new int[4];

            for(int i = 0; i < this.vet_peoes.Length; i++)
            {
                posicoes[i] = this.vet_peoes[i].coluna;
            }
