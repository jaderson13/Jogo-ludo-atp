...
namespace trabalho_ludo
{
    internal class Jogador
    {
        ...
        public bool Validar_jogada(int jogada_escolhida, int peao_escolhido_coluna, out bool sair_da_casa,ref bool peao_vitoria)// ver se a jogada existe e se é possível
        {
            sair_da_casa = false;
            ...
            if (continuar)
            {
                ...
                else if (peao_escolhido_coluna + jogada_escolhida <= 57)
                {
                    ...
                    if (peao_escolhido_coluna + jogada_escolhida == 57)
                    {
                        peao_vitoria = true;
                    }
                }
        ...
        public void Mover_peao(int peao_escolhido, int num_dado, out bool checkpoint,bool sair_da_casa)
        {
            ...
            else if (!sair_da_casa)
            {
                for (int i = 0; i < this.vet_peoes.Length; i++)
                {
                    if (this.vet_peoes[i].numero == peao_escolhido)
                    {
                        ...
                        {
                            if (coluna_peao == vet_checkpoints[j])
                            {
                                checkpoint = true;
                                break;
                            }
        ...
        public bool Validar_turno()// testar se pelo menos existe uma jogada válida no turno seguinte
        {
        ...
        public bool Jogar_dado(ref bool jogada_extra)
        {
            ...
            {
                passar_vez = true;
                Console.WriteLine("\nVocê perdeu a vez! Digite algo para continuar: ");
